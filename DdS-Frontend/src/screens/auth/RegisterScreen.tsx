...
export default function RegisterScreen({ navigation }: any) {
  ...
  const handleRegister = async () => {
    if (!email || !password || !role || !nombre || !telefono || (role === 'conductor' && (!estado || !licencia || !vehiculo))) {
      ...
      <View style={{ flexDirection: 'row', marginBottom: 20 }}>
        {['despachador', 'conductor'].map((r) => (
          <TouchableOpacity
            ...
            onPress={() => setRole(r as 'despachador' | 'conductor')}
          >
            <Text style={{ color: role === r ? '#fff' : '#000' }}>
